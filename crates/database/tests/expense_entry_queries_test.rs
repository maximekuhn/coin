...
use domain::{testutils::expense_entry::TestExpenseEntry, types::expense_entry_id::ExpenseEntryId};
...
async fn create_err_pk_violation(pool: SqlitePool) {
    ...
    let err = database::queries::expense_entry::create(
        &mut tx,
        &fixtures::expense_entries::john_and_bill_shared_shared_expenses_expense_1(),
    )
    .await
    .unwrap_err();
    match err {
...
async fn get_by_id_found(pool: SqlitePool) {
    let mut tx = pool.begin().await.unwrap();
    let expected = fixtures::expense_entries::john_and_bill_shared_shared_expenses_expense_1();
