...
use crate::infra::{ctx::TestContext, dates, db::build_test_database};
...
async fn happy_path() -> anyhow::Result<()> {
    ...
    let expense_id = ctx
        .expense_entries()
        .create_expense_for_all_group_members(group_id, bob_id, 128, bob_id, dates::jan_08_2025())
        .await?;

    // Then
    let expense_entry = ctx
        .expense_entries()
        .assert_expense_has_a_single_entry(expense_id)
        .await?;

    assert_eq!(bob_id, expense_entry.payer_id.value());
    assert_eq!(
    ...
    assert_eq!(bob_id, expense_entry.author_id.value());
    assert_eq!(dates::jan_08_2025(), expense_entry.occurred_at);
...
async fn happy_path_list_participants() -> anyhow::Result<()> {
    ...
    let expense_entry = ctx
        ...
        .assert_expense_has_a_single_entry(expense_id)
        .await?;
    assert_eq!(alice_id, expense_entry.payer_id.value());
    assert_eq!(bob_id, expense_entry.author_id.value());
    assert_eq!(
        HashSet::from_iter(vec![UserId::new(bob_id)?]),
        expense_entry.participants
    );
...
async fn negative_total() -> anyhow::Result<()> {
    ...
    // Then
    assert_eq!(
        CreateExpenseError::InvalidTotal.to_string(),
        err.to_string()
    );
...
#[tokio::test]
async fn author_not_in_group() -> anyhow::Result<()> {
    let db_pool = build_test_database(FILE, "author_not_in_group").await?;
    let ctx = TestContext::new(db_pool);
    ...
    let group_id = ctx
        ...
        .await?;
    ctx.groups()
        .add_member(group_id, alice_id, charlie_id)
        .await?;
    ...
    // Then
    assert_eq!(
        CreateExpenseError::AuthorNotInGroup.to_string(),
        err.to_string()
    );
